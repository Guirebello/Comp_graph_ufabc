...
float rotationSpeed = 1.0f; // Velocidade da rotação
bool isRotating = true;     // Controle para pausa de rotação
...
void keyboard(unsigned char key, int x, int y) {
    switch (key) {
        case 'r': rotationSpeed = fabs(rotationSpeed); break;  // Define rotação sentido horário
        case 'l': rotationSpeed = -fabs(rotationSpeed); break; // Define rotação sentido anti-horário
        case 'p': isRotating = !isRotating; break; // Pausa ou retoma a rotação
        ...
        case '1': rotationSpeed = (rotationSpeed > 0 ? 0.5f : -0.5f); break;
        case '2': rotationSpeed = (rotationSpeed > 0 ? 1.0f : -1.0f); break;
        case '3': rotationSpeed = (rotationSpeed > 0 ? 2.0f : -2.0f); break;
    }
    glutPostRedisplay();
}
